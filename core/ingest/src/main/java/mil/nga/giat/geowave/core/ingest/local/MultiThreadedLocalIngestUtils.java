...
public class MultiThreadedLocalIngestUtils
{
	...
	/**
	 * Create a thread pool for usage with multi-threaded ingest. This will
	 * create 'nthreads' which can all pull off of a task queue of 'boundedSize'
	 * (number of GeoWaveData items outstanding)
	 * 
	 * @param nthreads
	 * @param boundedSize
	 * @return
	 ...
	 * @param ingestOptions
	 * @param ingestPlugin
	 *            - the format plugin (to retrieve adapter)
	 * @param indexProvider
	 * @param ingestRunData
	 * @param nthreads
	 * @throws IOException
	...
	public static <T> void ingest(
		...
		ExecutorService service = getFixedThreadPool(
				nthreads,
				MAX_QUEUE_SIZE);
		...
		try (CloseableIterator<?> geowaveDataIt = ingestPlugin.toGeoWaveData(
			...
			while (geowaveDataIt.hasNext()) {
				...
				service.submit(new MultiThreadedIngestUnitOfWork(
						ingestRunData,
						geowaveData,
						dimensionalityIndexMap,
						requiredIndexMap));
			}
		}
		service.shutdown();
		try {
			...
			throw new RuntimeException(
					"Ingest failed to terminate after 250 seconds");
		}
	}
	 ...
	 */
	public static class MultiThreadedIngestUnitOfWork implements
			Runnable
	{
		...
		public MultiThreadedIngestUnitOfWork(
				IngestRunData ingestRunData,
				GeoWaveData<?> geowaveData,
				Map<ByteArrayId, IndexWriter> dimensionalityIndexMap,
